 ...
 *
 *  `partnerEmail` רשות: פותח לבן/בת הזוג חשבון עם אותה סיסמה וגישה מלאה
 *  לאותה חתונה. מוחזר `partner` כשהצירוף בוצע, כדי להציג אישור.
 */
export async function signUp(
  ...
  return {
    user: data.user,
    joinedWeddingId: data.joinedWeddingId ?? null,
...
export function authErrorMessage(err, mode) {
  switch (err?.code) {
    ...
    case "timeout":
      return "המערכת לא השיבה בזמן. נסו שוב בעוד רגע.";
    case "network_error":
    ...
    case "database_unavailable":
      return "המערכת מתעוררת. המתינו כמה שניות ונסו שוב.";
    default:
